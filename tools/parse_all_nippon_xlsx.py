#!/usr/bin/env python3
"""Run parse_nippon_xlsx over every downloaded month, report a compact
per-month/per-fund trust summary (weight sum vs GRAND TOTAL) so layout drift
across 14 years of archive is visible at a glance rather than eyeballing 165
runs. Mirrors parse_all_ppfas_xlsx.py / structure exactly."""
import json
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))
from parse_nippon_xlsx import parse_sheet, load_workbook_rows
