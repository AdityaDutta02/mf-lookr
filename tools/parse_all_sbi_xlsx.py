#!/usr/bin/env python3
"""Run parse_sbi_xlsx over every downloaded (period, scheme) file, report a
compact per-file trust summary (weight sum vs GRAND TOTAL (AUM)) so layout
drift across ~120-140 scheme types / dozens of months is visible at a glance
rather than eyeballing thousands of individual runs. Mirrors
parse_all_hdfc_xlsx.py's reporting shape exactly (SBI is also one-file-per-
scheme, unlike PPFAS's one-workbook-many-sheets)."""
import json
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))
from parse_sbi_xlsx import parse_sheet, load_workbook_rows
