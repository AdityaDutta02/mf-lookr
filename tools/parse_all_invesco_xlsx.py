#!/usr/bin/env python3
"""Run parse_invesco_xlsx over every downloaded (period, scheme) file, report
a compact per-file trust summary (weight sum vs Grand Total) so layout drift
across the ~40 scheme-slots / dozens of months is visible at a glance rather
than eyeballing thousands of individual runs. Mirrors
parse_all_hdfc_xlsx.py's reporting shape."""
import json
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))
from parse_invesco_xlsx import parse_sheet, load_workbook_sheets
    ...
    for period in periods:
        ...
        for xlsx_path in sorted(period_dir.glob("*.xlsx")):
            ...
            for sheet_name, rows in sheets.items():
                ...
                # last-write-wins on a within-period duplicate fund name (same
                # tradeoff parse_all_hdfc/parse_all_ppfas make); only flagged
                # via mismatches if trust band actually fails.
