// One-shot bootstrap: inserts the locally-parsed HDFC bundle (built by
// tools/build_dataset_hdfc.py — see tools/README.md) into amcs/funds/disclosures
...
// amc_slug: "hdfc" only. This is a locked constraint: a HDFC seed run must never
// touch a ppfas-slugged row. Compare the dbList/dbDelete calls here against
// seed-ppfas/route.ts's — same shape, only the slug differs.
//
// disclosures are fully SCRUBBED (delete every existing hdfc row, then insert
...
const bundle: Bundle = JSON.parse(
  fs.readFileSync(path.join(process.cwd(), "app/api/admin/seed-hdfc/data.json"), "utf8")
);
...
// dbBulkInsert caps at 1000 rows/call (see lib/db.ts). PPFAS's whole bundle (223
// disclosures) fits in one call; HDFC's (~100+ schemes x many months) doesn't, so
// every bulk insert here is chunked. Keep the chunk size comfortably under the
// 1000-row cap.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Hard-scoped to amc_slug: "hdfc" — must never read/delete a ppfas-slugged row.
    const existing = await dbList<DisclosureRow>("disclosures", { amc_slug: "hdfc" }, token);
