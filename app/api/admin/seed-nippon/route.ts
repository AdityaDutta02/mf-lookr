// One-shot bootstrap: inserts the locally-parsed Nippon India bundle (built by
// tools/build_dataset_nippon.py — see tools/README.md) into amcs/funds/disclosures
// via the gateway. Mirrors app/api/admin/seed-hdfc/route.ts's pattern EXACTLY —
// full scrub-then-reload, chunked bulk inserts — but every read/delete/insert
// below is hard-scoped to amc_slug: "nippon" only. This is a locked constraint:
// a Nippon seed run must never touch a ppfas- or hdfc-slugged row. Compare the
// dbList/dbDelete calls here against seed-ppfas/route.ts's and seed-hdfc/route.ts's
// — same shape, only the slug differs.
//
// disclosures are fully SCRUBBED (delete every existing nippon row, then insert
...
//
// data.json here is scoped to the last ~18 months (2025-01 .. 2026-06), not
// Nippon's full 2012-09-onward archive — tools/out/nippon_bundle.json (built by
// running build_dataset_nippon.py with no period args) has the full history if
...
const bundle: Bundle = JSON.parse(
  fs.readFileSync(path.join(process.cwd(), "app/api/admin/seed-nippon/data.json"), "utf8")
);
