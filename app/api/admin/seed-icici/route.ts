// One-shot bootstrap: inserts the locally-parsed ICICI Prudential bundle
// (built by tools/build_dataset_icici.py — see tools/README.md) into
// amcs/funds/disclosures via the gateway. Mirrors
// app/api/admin/seed-ppfas/route.ts's and app/api/admin/seed-hdfc/route.ts's
// pattern EXACTLY — full scrub-then-reload — but every read/delete/insert
// below is hard-scoped to amc_slug: "icici" only. This is a locked
// constraint: an ICICI seed run must never touch a ppfas- or hdfc-slugged
// row (or any other AMC's rows). Compare the dbList/dbDelete calls here
// against seed-hdfc/route.ts's — same shape, only the slug differs.
//
// disclosures are fully SCRUBBED (delete every existing icici row, then
...
const bundle: Bundle = JSON.parse(
  fs.readFileSync(path.join(process.cwd(), "app/api/admin/seed-icici/data.json"), "utf8")
);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Hard-scoped to amc_slug: "icici" — must never read/delete a
    // differently-slugged row.
    const existing = await dbList<DisclosureRow>("disclosures", { amc_slug: "icici" }, token);
