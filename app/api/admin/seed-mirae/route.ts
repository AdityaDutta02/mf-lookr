// One-shot bootstrap: inserts the locally-parsed Mirae Asset bundle (built by
// tools/build_dataset_mirae.py — see tools/README.md) into amcs/funds/disclosures
// via the gateway. Mirrors app/api/admin/seed-ppfas/route.ts's and
// seed-hdfc/route.ts's pattern EXACTLY — full scrub-then-reload — but every
// read/delete/insert below is hard-scoped to amc_slug: "mirae" only. This is a
// locked constraint: a Mirae seed run must never touch a ppfas- or
// hdfc-slugged row. Compare the dbList/dbDelete calls here against those two
// route.ts's — same shape, only the slug differs.
//
// disclosures are fully SCRUBBED (delete every existing mirae row, then insert
// fresh) on every run, not just replaced for the periods this bundle happens to
// cover — see seed-ppfas/route.ts's comment for why a partial replace isn't
// safe. amcs/funds are stable identity data — plain dbBulkInsert is fine there,
...
const bundle: Bundle = JSON.parse(
  fs.readFileSync(path.join(process.cwd(), "app/api/admin/seed-mirae/data.json"), "utf8")
);
...
// dbBulkInsert caps at 1000 rows/call (see lib/db.ts). Mirae has 100+ schemes
// across many months, same as HDFC — chunk every bulk insert, comfortably
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Hard-scoped to amc_slug: "mirae" — must never read/delete a ppfas- or
    // hdfc-slugged row.
    const existing = await dbList<DisclosureRow>("disclosures", { amc_slug: "mirae" }, token);
